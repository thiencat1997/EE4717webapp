<!DOCTYPE html>
<html>
<head>
	<title>MedArt Clinic</title>	
	  <link rel="stylesheet" href="styles.css">
</head>
<style>
img{
	width:100%;
	border-radius: 20px;
}

.row{
	text-align: justify; 
	padding:30px ;
	padding-left: 100px ;
	padding-bottom: 50px ;
	padding-right: 100px;
}

h3{
	font-size: 40px;
	font-weight: bold;
}

h1{
	font-size: 60px;
}
</style>
<body>
		
	<div class="banner-container-2" style="background-image: url('media/contact-bg.png');">		
		<div style="width:1500px; margin: auto" >

		<?php include_once 'subhtml/navbar.php'; ?>
		
		  <div class="banner-text-2">
			Services
		  	  </div>
		</div>



	</div>
		<br>

<div class="container" style="background:transparent; margin-top:50px; padding-top: 100px;padding-bottom: 50px;
 margin-bottom: 100px; color:white;
    background-image: linear-gradient(270deg, rgb(50, 200, 250) 0%, rgb(88, 125, 228) 100%);">
    <div style="text-align: center; padding-bottom: 50px">
	    <h1>Our Services</h1>
	    <p>We have a wide variety of high class equipment, capable of delivering tip top services. Lorem ipsum dolor sit amet, consectetur<br> adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus</p>
    </div>
	<div class="row" style="">
			<div class="column">
			  	<div class="column-left">
					<h3>
						<img class="service" src="media/teeth.png">
						Service 1
						</h3>	
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
				</div>
			  </div>
			  <div class="column">
			  	<div class="column-middle">
			  		<h3>
						<img class="service" src="media/blood-donation-2.png">
						Service 2
						</h3>
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
			  </div>
			  </div>
			  <div class="column">
			  	<div class="column-right">
			  		<h3>
						<img class="service" src="media/bones.png">
						Service 3
						</h3>

				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
					</p>
				  </div>
			  </div>
		</div>
	<div class="row">
			<div class="column">
			  	<div class="column-left">
					<h3>
						<img class="service" src="media/glasses.png">
						Service 4
						</h3>
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
				</div>
			  </div>
			  <div class="column">
			  	<div class="column-middle">
			  		<h3>
						<img class="service" src="media/scanner.png">
						Service 5
						</h3>	
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
			  </div>
			  </div>
			  <div class="column">
			  	<div class="column-right">
			  		<h3>
						<img class="service" src="media/surgery.png">
						Service 6
						</h3>	

				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
					</p>
				  </div>
			  </div>
		</div>
	<div class="row">
			<div class="column">
			  	<div class="column-left">
					<h3>
						<img class="service" src="media/blood-sample-2.png">
						Service 7
						</h3>
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
				</div>
			  </div>
			  <div class="column">
			  	<div class="column-middle">
			  		<h3>
						<img class="service" src="media/stomach-2.png">
						Service 8
						</h3>	
				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
				</p>
			  </div>
			  </div>
			  <div class="column">
			  	<div class="column-right">
			  		<h3>
						<img class="service" src="media/cardiogram.png">
						Service 9
						</h3>	

				<p style="margin:0">
				Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec malesuada lorem maximus mauris scelerisque, at rutrum nulla dictum. Ut ac ligula sapien. Suspendisse cursus
					</p>
				  </div>
			  </div>
		</div>

		</div>

<?php include_once 'subhtml/signUpBanner.php'; ?>
<?php include_once 'subhtml/footer.php'; ?>

</body>
</html>
