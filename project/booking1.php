...
<?php
    ...
    if (isset($_POST['doctor']) && isset($_POST['date'])){
      ...
      */
      $query ="
           SELECT t.* FROM timeslot AS t WHERE t.SlotID NOT IN(
            SELECT b.SlotID FROM booking as b 
            WHERE b.Date='$date' AND b.DrName='$doctor');
      ";
      $result = mysqli_query($conn, $query);
      while ($rows = mysql_fetch_assoc($result)){
        echo $rows['SlotID'].":".['time'];
      }
    }
          ...
          <div class="bookform">
            ...
            <div>
              ...
              <select  class="time-picker" name="time" id="time">
                <?php
                  while ($rows = mysql_fetch_assoc($result)){
                      echo "<option value= ' ".$rows['SlotID']." '> ".$rows['time']."</option>";
                  }
                ?>
              </select>
